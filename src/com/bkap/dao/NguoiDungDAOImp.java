...
public class NguoiDungDAOImp implements INguoiDungDAO{
    ...
    public void add(NguoiDung nd) {
        SqlConnection.executeUpdate("INSERT INTO NguoiDung VALUES(?,?,?,?,?,?)", nd.getId(), nd.getTenNguoiDung(), nd.getEmail(), nd.getMatKhau(), nd.getSdt(), nd.getRole());
    }

    @Override
    public void edit(NguoiDung nd) {
        SqlConnection.executeUpdate("UPDATE NguoiDung SET tenNguoiDung = ?, email = ?, matKhau = ?, sdt = ?, role = ? WHERE id = ?", nd.getTenNguoiDung(), nd.getEmail(), nd.getMatKhau(), nd.getSdt(), nd.getRole(), nd.getId());
    }
